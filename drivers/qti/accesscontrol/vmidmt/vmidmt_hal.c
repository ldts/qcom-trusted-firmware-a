				      ...
				      uint32_t vmid)
{
	...
	field_index = vmid % HAL_VMIDMT_PERMS_PER_FIELD;
	word_index  = vmid / HAL_VMIDMT_PERMS_PER_FIELD;

	perm_word = cfg->auVMIDPerm[word_index];
	shift = field_index * HAL_VMIDMT_PERM_WIDTH;

	return (HAL_vmidmt_AccessType)
		((perm_word >> shift) & HAL_VMIDMT_PERM_MASK);
}
				       ...
				       HAL_vmidmt_AccessConfigType *cfg)
{
	...
	for (size_t vmid = 0; vmid <= HAL_VMIDMT_MAX_VMID; vmid++) {
		if (!IS_MASK_BIT_SET(cfg, vmid))
			continue;

		/* only set bits for full access masters */
		if (get_permissions(cfg, vmid) == HAL_VMIDMT_FULL_ACCESS)
			acr |= 1 << vmid;
	}

	VMIDMT_OUTM(addr, VMIDMTACR, cfg->auVMID[0], acr);
	val = VMIDMT_INM(addr, VMIDMTACR, cfg->auVMID[0]);
	if (val != acr)
					   ...
					   bool sec)
{
	...
	if (sec) {
		VMIDMT_OUTF(p->uBaseAddr, SCR0, CLIENTPD, readback);
		readback = VMIDMT_INF(p->uBaseAddr, SCR0, CLIENTPD);
	} else  {
		VMIDMT_OUTF(p->uBaseAddr, NSCR0, CLIENTPD, readback);
		readback = VMIDMT_INF(p->uBaseAddr, NSCR0, CLIENTPD);
	}

	return readback ? HAL_VMIDMT_READ_WRITE_MISMATCH : HAL_VMIDMT_NO_ERROR;
}
...
static
HAL_vmidmt_Status vmidmt_validate_ctx_args(const HAL_vmidmt_InfoType *info,
				  uint32_t stream,
...
static HAL_vmidmt_Status vmidmt_update_ssdrn(const HAL_vmidmt_InfoType *info,
					     uint32_t stream,
					     bool is_secure)
{
	uint32_t ssd_index;
	...
	ssdrn_val = is_secure ? ~bit_mask : bit_mask;

	VMIDMT_OUTMI(info->uBaseAddr, SSDRn, ssd_index, bit_mask, ssdrn_val);

	rd_val = VMIDMT_INMI(info->uBaseAddr, SSDRn, ssd_index, bit_mask);

	if (rd_val != (ssdrn_val & bit_mask))
		return HAL_VMIDMT_READ_WRITE_MISMATCH;

	return HAL_VMIDMT_NO_ERROR;
}

static HAL_vmidmt_Status vmidmt_update_smr(const HAL_vmidmt_InfoType *info,
					   uint32_t stream,
					   bool is_secure,
					   uint32_t *index_out)
{
	...
	for (i = 0; i < (int32_t)entry_count; i++) {
		if (VMIDMT_INI(info->uBaseAddr, SMRn, (uint32_t)i) == smr_val)
			break;
	}

	if (i < (int32_t)entry_count) {
		...
		if ((uint32_t)i < ns_smr_count) {
			for (; (uint32_t)i < ns_smr_count - 1U; i++) {
				rc = swap_context(info, (uint32_t)i + 1U,
						  (uint32_t)i);
				if (rc != HAL_VMIDMT_NO_ERROR)
					return rc;
			}
			...
			VMIDMT_OUTF(info->uBaseAddr, SCR1,
				    NSNUMSMRGO, ns_smr_count);
		...
		if (is_secure) {
			for (i = (int32_t)entry_count - 1; i >= 0; i--) {
				if (VMIDMT_INFI(info->uBaseAddr, SMRn,
						(uint32_t)i, VALID) != 1U)
					break;
...
write_and_adjust:
	VMIDMT_OUTI(info->uBaseAddr, SMRn, (uint32_t)i, smr_val);

	rd_val = VMIDMT_INI(info->uBaseAddr, SMRn, (uint32_t)i);
	if (rd_val != smr_val)
		return HAL_VMIDMT_READ_WRITE_MISMATCH;
					...
					const HAL_vmidmt_ContextConfigType *cfg)
{
	...
	rc = vmidmt_validate_ctx_args(info, stream, cfg, &is_secure);
	if (rc != HAL_VMIDMT_NO_ERROR)
		return rc;

	rc = vmidmt_update_ssdrn(info, stream, is_secure);
	if (rc != HAL_VMIDMT_NO_ERROR)
		return rc;

	if (info->devParams.bStreamMatchSupport) {
		rc = vmidmt_update_smr(info, stream, is_secure, &ctx_index);
		if (rc != HAL_VMIDMT_NO_ERROR)
			return rc;
	} else {
		ctx_index = 0;
