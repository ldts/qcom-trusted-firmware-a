...
#define IS_MASK_BIT_SET(p, v)						\
	(!!((p)->auVMID[(v) / HAL_VMIDMT_MASKS_PER_FIELD] &		\
	    (1U << (((v) % HAL_VMIDMT_MASKS_PER_FIELD) *		\
		    HAL_VMIDMT_PERM_WIDTH))))
...
/* VMIDMT non-secure allocate configuration options (secure access only) */
typedef enum {
...
/* Redirect cacheable-non-shareable configuration */
typedef enum {
...
/* Redirect cacheable-inner-shareable configuration */
typedef enum {
...
/* Redirect cacheable-outer-shareable configuration */
typedef enum {
...
/* Default VMIDMT configuration (bypass + access control + RPU perms) */
typedef struct {
...
/* Default secure VMIDMT configuration (includes security extensions) */
typedef struct {
...
/* Context configuration (stream mapping + group->context) */
typedef struct {
...
/* Initialize VMIDMT.
 * Returns status and optionally fills an error string on failure.
 */
