...
} access_control_error_t;

#define MSM_INT_XPU_NON_SEC_DESC    "SPI XPU NonSec"
#define MSM_INT_XPU_SEC_DESC        "SPI XPU Sec"

static int xpu_err_non_sec_ctx = XPU_ERR_NON_SEC_CTX;
static int xpu_err_sec_ctx = XPU_ERR_SEC_CTX;

static int update_master_side_mpu(xpuInstanceType *xpu_instance,
				  ...
				  uint32_t perm_w)
{
	uint64_t last_index  = xpu_instance->part_range_arr_size;
	uint64_t first_index = last_index - dynamic_partition_count;

	RGPrtitionRangeType *range_base  = xpu_instance->partition_range;
	RGDomainOwnershipType *owner_base = xpu_instance->rg_owner;

	RGPrtitionRangeType *range_iter;
	RGDomainOwnershipType *owner_iter;

	RGPrtitionRangeType *selected_range = NULL;
	RGDomainOwnershipType *selected_owner = NULL;

	uint64_t idx;

	range_iter = range_base + first_index;
	owner_iter = owner_base + first_index + 1; /* +1 for unmapped entry */

	for (idx = first_index; idx < last_index;
	      idx++, range_iter++, owner_iter++) {

		/* Check if this RG already covers the requested range */
		if (range_iter->start_addr == start_addr &&
		    range_iter->end_addr   == end_addr) {

			/*
			 * Free RG for later use when only NS access is needed.
			 */
			if (domain == APPS_NS_DOMAIN) {
				...
				INFO("freeing RG for xpu 0x%lx idx:%llu\n",
				     (unsigned long)xpu_instance->xpu_base_addr,
				     (unsigned long long)idx);
			}

			selected_range = range_iter;
			selected_owner = owner_iter;
			break;
		}
		 ...
		 */
		if (owner_iter->owner_domain == NO_DOMAIN &&
		    range_iter->start_addr    == 0xfffffffful &&
		    range_iter->end_addr      == 0xfffffffful) {
	...
	if (!selected_range || !selected_owner) {
		ERROR("No free RG xpu addr : 0x%lx",
		      (unsigned long)xpu_instance->xpu_base_addr);
		return 1;
	}

	selected_owner->owner_domain = domain;
	selected_range->start_addr   = start_addr;
	selected_range->end_addr     = end_addr;

	return xpu_lock_down_assets_dynamic(xpu_instance,
					    1,
					    xpu_instance->xpu_id,
					    selected_range->rg_num,
					    perm_r,
					    perm_w);
}

static int mpu_master_mpus_range(deviceType dev_type,
				 domainType domain,
				 uintptr_t start_addr,
				 uintptr_t end_addr,
				 uint32_t perm_r,
				 uint32_t perm_w)
{
	mpuRangesType *range = msm_mpu_ranges;
	uint32_t i, j;
	int ret = 0;
...
static uint64_t mem_assign(const qti_accesscontrol_mem_t *mem_regions,
			   ...
			   const uint32_t *src_vm_list,
			   uint32_t src_vm_count,
			   const qti_accesscontrol_perm_t *dst_vm_list,
			   uint32_t dst_vm_count)
{
	access_control_error_t result = AC_SUCCESS;
	uint32_t write_perm_domain = NO_DOMAIN;
	uint32_t read_perm_domain = NO_DOMAIN;
	uint32_t vm_also_in_other_list = 0;
	domainType domain = NO_DOMAIN;
	deviceType device = 0;
	...
	const qti_accesscontrol_mem_t *mem_region;
	uint32_t src_index;
	uint32_t dst_index;
	uint32_t mem_index;
	...
	for (src_index = 0; src_index < src_vm_count; src_index++, src_vm++) {
		...
		dst_vm = dst_vm_list;
		dst_vm_limit = dst_vm_list +
			       ((dst_vm_count < (uint32_t)AC_VM_LAST) ?
				dst_vm_count : (uint32_t)AC_VM_LAST);

		for (dst_index = 0; dst_vm < dst_vm_limit;
		      dst_index++, dst_vm++) {
			if (dst_vm->dst_vm != src_vm_id)
				continue;

			vm_also_in_other_list = 1;
			break;
		}

		if (vm_also_in_other_list) {
			vm_also_in_other_list = 0;
			continue;
		}
		...
		if (src_vm_id == AC_VM_MSS_MSA)
			...
			result = AC_ERR_XPU_TYPE_NOT_SUPPORTED;
			goto out;
		}
	...
	for (dst_index = 0; dst_index < dst_vm_count; dst_index++, dst_vm++) {
		...
		for (src_index = 0; src_index < src_vm_count;
		      src_index++, src_check++) {
		...
		/* Permissions */
		if (dst_vm_perm & AC_PERM_W)
			write_perm_domain = MSA_DOMAIN;

		if (dst_vm_perm & AC_PERM_R)
	...
	for (mem_index = 0; mem_index < mem_region_count;
		...
		int rc = mpu_master_mpus_range(device,
					       domain,
					       region_base,
					       region_end,
					       read_perm_domain,
					       write_perm_domain);
		if (rc) {
			result = AC_ERR_XPU_ADD_MAPPING_FAILED;
			goto out;
		}
	}
...
static int xpu_register_interrupts(void)
{
	...
	err = qti_interrupt_svc_register(QTISECLIB_INT_ID_XPU_SEC,
					 xpu_isr, &xpu_err_sec_ctx);
	if (err)
		return err;

	err = qti_interrupt_svc_register(QTISECLIB_INT_ID_XPU_NON_SEC,
					 xpu_isr, &xpu_err_non_sec_ctx);
	if (err)
		qti_interrupt_svc_unregister(QTISECLIB_INT_ID_XPU_NON_SEC);
...
{
	const xpu_intr_reg_dtls_t *nsec = xpu_non_sec_intr_en_reg;
	const xpu_intr_reg_dtls_t *sec = xpu_sec_intr_en_reg;
	const uint32_t val = 0xFFFFFFFF;
	uintptr_t addr;
	uint32_t mask;

	for (size_t i = 0; i < MSM_XPU_ERR_INT_REG_NUM; i++) {
		if (nsec) {
			addr = nsec->xpu_intr_reg_addr;
			mask = nsec->xpu_intr_reg_mask;
			mmio_write_32(addr, val & mask);
			nsec++;
		}

		if (sec) {
			addr = sec->xpu_intr_reg_addr;
			mask = sec->xpu_intr_reg_mask;
			mmio_write_32(addr, val & mask);
			sec++;
		}
	}
