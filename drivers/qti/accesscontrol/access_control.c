				  ...
				  uint32_t perm_w)
{
	...
	RGDomainOwnershipType *found_owner = NULL;
	RGPrtitionRangeType *found_range = NULL;
	RGDomainOwnershipType *owner;
	RGPrtitionRangeType *range;
	uint64_t idx;
...
static ac_error_t process_sources(const uint32_t *src_vm_list,
...
static ac_error_t process_destinations(const uint32_t *src_vm_list,
			uint32_t src_vm_count,
			const qti_accesscontrol_perm_t *dst_vm_list,
			uint32_t dst_vm_count,
			deviceType *device,
...
static ac_error_t assign_regions(const qti_accesscontrol_mem_t *mem_regions,
				 ...
				 uint32_t write_perm_domain)
{
	...
	for (mem_index = 0U; mem_index < mem_region_count;
		...
		rc = mpu_master_mpus_range(device,
					   domain,
					   region_base,
					   region_end,
					   read_perm_domain,
			   ...
			   uint32_t dst_vm_count)
{
	...
	result = process_sources(src_vm_list, src_vm_count,
				 dst_vm_list, dst_vm_count,
				 &device, &domain);
	if (result != AC_SUCCESS)
		goto out;

	result = process_destinations(src_vm_list, src_vm_count,
				      dst_vm_list, dst_vm_count,
				      &device, &domain,
				      &read_perm_domain,
				      &write_perm_domain);
	...
	result = assign_regions(mem_regions, mem_region_count,
				device, domain,
				read_perm_domain,
				write_perm_domain);
	if (result != AC_SUCCESS)
...
static void enable_interrupts(const xpu_intr_reg_dtls_t *nsec,
			      const xpu_intr_reg_dtls_t *sec)
{
